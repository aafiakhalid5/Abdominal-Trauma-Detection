...
class FaceDetector:
    ...
    # def align_face_with_landmarks(self, image, landmarks):
    #     src_points = np.array([landmarks['left_eye'], landmarks['right_eye'], landmarks['nose']], dtype=np.float32)
    #     dst_points = np.array([
    #         [0.3 * self.aligned_image_size, 0.4 * self.aligned_image_size],  # Left eye
    #         [0.7 * self.aligned_image_size, 0.4 * self.aligned_image_size],  # Right eye
    #         [0.5 * self.aligned_image_size, 0.6 * self.aligned_image_size]  # Nose
    #     ], dtype=np.float32)
    #
    #     M = cv2.getAffineTransform(src_points, dst_points)
    #
    #     aligned_face = cv2.warpAffine(image, M, (self.aligned_image_size, self.aligned_image_size))
    #
    #     return aligned_face
